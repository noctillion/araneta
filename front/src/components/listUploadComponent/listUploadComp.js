...
import FileUploadNum from "../fileupload/FileUploadNum";
...
const ListUploadComp = () => {
  const { setDataToProviderClicked, forNewNet, setDataToProviderCorrNodes } =
    useContext(NameContext);
  const [newUserInfo, setNewUserInfo] = useState([]);
  const [newData, setNewData] = useState([]);
  const [newInitialHold, setNewInitialHold] = useState([]);
  const [nodeCorrQuery, setNodeCorrQuery] = useState([]);

  const updateUploadedFiles = (files) =>
    setNewUserInfo({ ...newUserInfo, profileImages: files });
  console.log(newUserInfo, "newUserINNNNFOOO");

  useEffect(() => {
    if (forNewNet.nodes !== undefined) {
      let nodNetR = forNewNet.nodes.map((node) => {
        return node.name2;
      });
      let fusiNod = nodNetR.join();
      setNodeCorrQuery(fusiNod);
    }
  }, [forNewNet]);
  ...
  const onSubmit = async (data) => {
    console.log(data, "objectAQWWWWW");
    const formData = new FormData();
    formData.append("file", data.profileImages[0]);
    formData.append("username", nodeCorrQuery);
    const res = await fetch("http://localhost:5000/listsexp", {
      method: "POST",
      body: formData,
    }).then((res) => res.json());
