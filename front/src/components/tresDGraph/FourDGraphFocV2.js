...
export const FocusGraphDos = () => {
  ...
  // Return a solid node inside a transparent one to make a glow effect.
  var sphereMesh = function (node) {
    // Glow sphere mesh.
    const glowSize = 7;
    const geometry = new THREE.SphereBufferGeometry(glowSize, 32, 32);
    ...
    const geometrySolid = new THREE.SphereBufferGeometry(5, 32, 32);
    ...
    //outlineMesh1.position = glowEffect.position;
    outlineMesh1.scale.multiplyScalar(1.05);

    /*   const myText = new Text();
    myText.text = node.id;
    myText.fontSize = node.node_size1 / 500;
    myText.position.z = -2;
